...
Deno.serve(async (request) => {
  ...
  if (!ENDPOINT) {
    return json({ error: 'No image provider is configured.' }, 503)
  }
  ...
  try {
    ...
    if (!upstream.ok) {
      return json({ error: `Image generation failed (${upstream.status})` }, 502)
    }

    return json(await upstream.json())
  } catch (error) {
    console.error('ai-image failed', error)
    return json({ error: 'Image generation is unavailable right now.' }, 500)
  }
})
