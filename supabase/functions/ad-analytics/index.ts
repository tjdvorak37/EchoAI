...
Deno.serve(async (request) => {
  ...
  if (request.method === 'GET' && isProvider(callbackProvider)) {
    ...
    try {
      ...
      const accounts = callbackProvider === 'meta' ? await metaAccounts(accessToken) : callbackProvider === 'google' ? await googleAccounts(accessToken) : []
  ...
  try {
    ...
    if (body.action === 'report') {
      ...
      const campaigns = meta ? await metaReport(meta.access_token, meta.external_account_ids, days) : []
